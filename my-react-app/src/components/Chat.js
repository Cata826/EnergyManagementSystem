...
const Chat = () => {
  ...
  // const baseUrl = "http://localhost:8083/chat";
    const socketUrl = "http://chat.localhost/ws";
  const baseUrl = "http://chat.localhost/chat";
  const ADMIN_ID = "51b35185-e2e3-4fe4-ba37-42ac865e81ec";
  ...
  useEffect(() => {
    ...
    if (isAdmin) {
      axios
        ...
        //   },
        // })
        .get("http://users.localhost/person", {
          headers: {
            Authorization: `Bearer ${token}`, // Add the Bearer token here
          },
        })
