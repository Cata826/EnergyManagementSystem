...
//                .antMatchers("/broker/**").hasRole("ADMIN") // Doar ADMIN are acces la /device/**
//                .antMatchers("/consumer/**").hasRole("ADMIN") // Doar ADMIN are acces la /device/**
//                .anyRequest().authenticated()
//                .and()
//                .addFilterBefore(jwtRequestFilter, UsernamePasswordAuthenticationFilter.class);
//    }
    @Override
    protected void configure(HttpSecurity http) throws Exception {
        http.cors() // Activare suport CORS
                .and()
                .csrf().disable()
                .authorizeRequests()
                .antMatchers("/auth/login").permitAll() // Permite login
                .antMatchers("/broker/energy-consumption/**").permitAll()
                .antMatchers("/broker/**").hasRole("ADMIN") // Doar ADMIN are acces la /device/**
                .antMatchers("/consumer/checkMhec/person/**").permitAll()
                // Restrânge accesul la toate celelalte rute din /consumer doar pentru ADMIN
